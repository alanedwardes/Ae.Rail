...
namespace Ae.Rail.Services
{
    public class TrainsConsumerService : BackgroundService
    {
        ...
        private static IReadOnlyList<string> GetBritishRailClasses(PassengerTrainConsistMessage dto)
        {
            var classes = new List<string>();

            if (dto?.Allocation == null)
            {
                return classes;
            }

            // Units first ("U"): first 3 characters of ResourceGroupId
            foreach (var allocation in dto.Allocation)
            {
                var rg = allocation?.ResourceGroup;
                if (rg == null) continue;
                if (string.Equals(rg.TypeOfResource, "U", StringComparison.OrdinalIgnoreCase))
                {
                    var id = rg.ResourceGroupId;
                    if (!string.IsNullOrEmpty(id) && id.Length >= 3)
                    {
                        var unitClass = id.Substring(0, 3);
                        if (!classes.Contains(unitClass))
                        {
            ...
            // Locos ("L"): first 2 characters of each VehicleId
            foreach (var allocation in dto.Allocation)
            {
                var rg = allocation?.ResourceGroup;
                if (rg == null) continue;
                if (string.Equals(rg.TypeOfResource, "L", StringComparison.OrdinalIgnoreCase))
                {
                    if (rg.Vehicle != null)
                    {
                        foreach (var vehicle in rg.Vehicle)
                        {
                            var vehicleId = vehicle?.VehicleId;
                            if (!string.IsNullOrEmpty(vehicleId) && vehicleId.Length >= 2)
                            {
                                var locoClass = vehicleId.Substring(0, 2);
                                if (!classes.Contains(locoClass))
                                {
                                    classes.Add(locoClass);
                                }
                            }
                        }
                    }
        ...
        private static string GetPowerType(string britishRailClass)
        {
            if (string.IsNullOrWhiteSpace(britishRailClass))
            {
                return null;
            }

            if (!int.TryParse(britishRailClass, out var cls))
            {
                return null;
            }

            // LINX Table 8 mapping
            return cls switch
            {
                >= 1 and <= 70 => "Diesel",
                >= 71 and <= 96 => "Electric",
